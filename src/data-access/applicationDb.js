const model = require("./mongo/models/application");

const applicationDb = Object.freeze({
  insert,
  findAll,
  findById,
  findOne,
...
async function findAll({ filters, q, page, sort }) {
  ...
  if (q) {
    filter.$or = [
      ...
      { applicationname: { $regex: `.*${q}.*`, $options: "i" } }
    ];
  }
  ...
  const res = result.map((application) => {
    const { _id: id, ...info } = application;
    return { id, ...info };
  });
